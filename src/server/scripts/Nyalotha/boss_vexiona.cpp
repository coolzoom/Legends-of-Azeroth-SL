...
private:
	...
	{
		_JustReachedHome();
		me->DespawnCreaturesInArea2(NPC_ASCENDANT, 125.0f);
		me->DespawnCreaturesInArea2(NPC_GIFT_OF_THE_VOID, 125.0f);
		me->DespawnCreaturesInArea2(NPC_FANATICAL_CULTIST, 125.0f);
		me->DespawnCreaturesInArea2(NPC_SPELLBOUND_RITUALIST, 125.0f);
		me->DespawnCreaturesInArea2(NPC_DARK_GATEWAY, 125.0f);
		me->DespawnCreaturesInArea2(NPC_SINISTER_SOULCARVER, 125.0f);
		me->DespawnCreaturesInArea2(NPC_IRON_WILLED_ENFORCER, 125.0f);
		...
		case SPELL_HEART_OF_DARKNESS:
			...
			for (auto& targets : playerList)
			{
				if (!targets->SelectNearestPlayer(6.0f))
					targets->AddAura(SPELL_HEART_OF_DARKNESS_FEAR, targets);
			}
			break;
		}
	}
...
struct npc_gift_of_the_void : public ScriptedAI
{
	...
	void OnSpellClick(Unit* clicker, bool& /*result*/)
	{
		if (Player* player = clicker->ToPlayer())
		{
			...
			player->AddAura(SPELL_GIFT_OF_THE_VOID, player);
