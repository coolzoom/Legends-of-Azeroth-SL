...
private:
    ...
    void IsSummonedBy(WorldObject* summoner) override
    {
        ...
        {
            //player->GetSceneMgr().PlaySceneByPackageId2(SCENE_GORGROTHS_RITUAL);
...
class item_resizer : public ItemScript
{
public:
    item_resizer() : ItemScript("item_resizer") { }
...
struct npc_captain_garrick_156280 : public ScriptedAI
{
    ...
    {
        if (quest->GetQuestId() == QUEST_RIDE_OF_THE_SCIENTIFICALLY_ENHANCED_BOAR)
            player->SummonCreature(156595, TEMPSUMMON_MANUAL_DESPAWN, 60000, 0U); //10 minutes to despawn
    }
    ...
    void OnSpellClick(Unit* clicker, bool& spellClickHandled)
    {
        if (Player* player = clicker->ToPlayer())
        {
            if (player->hasQuest(QUEST_AN_END_TO_BEGINNINGS))
            {
                ...
                //player->GetSceneMgr().PlaySceneByPackageId2(SCENE_FLY_AWAY);
...
void AddSC_zone_exiles_reach()
{
    ...
    RegisterItemScript(item_resizer);
