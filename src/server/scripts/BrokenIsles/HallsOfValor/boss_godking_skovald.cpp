    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (me->IsInCombat())
        {
            if (powerTimer <= diff)
            {
                ...
                else if (!events.GetNextEventTime(EVENT_RAGNAROK))
...
struct npc_aegis_of_aggramar : public ScriptedAI
{
    ...
    void IsSummonedBy(WorldObject* summoner) override
    {
        ...
        me->SetTransformSpell(SPELL_AEGIS_SHIELD_PLAYER);
...
class spell_ragnarok : public AuraScript
{
    ...
    void OnPeriodic(AuraEffect const* aurEff)
    {
        ...
        {
            caster->CastSpell(caster, 193827);
            caster->CastSpell(caster, 202494);
        }
    }
...
struct npc_flame_of_woe : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        EventMap().ScheduleEvent(EVENT_FLAME_OF_WOE, 1000);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = EventMap().ExecuteEvent())
        {
            ...
            case EVENT_FLAME_OF_WOE:
                ...
                EventMap().ScheduleEvent(EVENT_SUMMON_AEGIS, 1000);
                break;
            }
        }
