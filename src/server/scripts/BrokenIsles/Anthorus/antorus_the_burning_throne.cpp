...
struct npc_garothi_annihilator_123398 : public ScriptedAI
{
    ...
    {
        EventMap().ScheduleEvent(SPELL_ANNIHILATION, 5s);
    }
...
struct npc_clobex_127732 : public ScriptedAI
{
    ...
    {
        Talk(1);
        EventMap().ScheduleEvent(SPELL_PYROGENICS, 5s);
        EventMap().ScheduleEvent(SPELL_FEL_FIREBALL, 5s);
        EventMap().ScheduleEvent(SPELL_SOULBURN, 5s);
    }
