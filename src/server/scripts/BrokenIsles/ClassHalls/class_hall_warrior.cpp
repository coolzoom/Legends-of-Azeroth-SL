...
class spell_class_hall_warrior_jump_teleport : public SpellScript
{
    ...
    void HandleTeleport(SpellEffIndex effIndex)
    {
        ...
        if (Player* player = GetHitPlayer())
        {
            ...
            player->TeleportTo(_loc.GetMapId(), _loc.GetPositionX(), _loc.GetPositionY(), _loc.GetPositionZ(), _loc.GetOrientation(), true);
...
struct npc_danica_the_reclaimer : public ScriptedAI
{
    ...
    void QuestAccept(Player* player, Quest const* /*quest*/) override
    {
        TempSummon* summon = player->SummonCreature(NPC_DANICA_THE_RECLAIMER, 1059.613f, 7224.605f, 100.4608f, 0.03462749f, TEMPSUMMON_MANUAL_DESPAWN, 0);
