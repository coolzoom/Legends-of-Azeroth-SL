...
public:
    ...
    bool Execute(uint64 /*time*/, uint32 /*diff*/)
    {
        if (_caster && _target && _caster->IsInWorld() && _target->IsInWorld())
            _caster->SendPlaySpellVisual(_target, _spellVisualId, 0.f, 0.f, 40.0f, false);
        return true;
    }
    ...
    struct boss_skorpyronAI : public BossAI
    {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (instance->GetData(DATA_CAGE_REMATCH) == DONE)
                instance->DoCompleteAchievement2(ACHIEVEMENT_CAGE_REMATCH);
        }
                ...
                case EVENT_FOCUSED_BLAST:
                    if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0, true))
                        if (auto crystalTail = me->GetVehicleKit()->GetPassenger(0))
                        {
                            ...
                            me->AttackStop();
                            me->SetFacingTo(me->GetAngle2(target));
...
public:
    ...
    struct npc_crystalline_scorpidAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/)
        {
            EventMap().ScheduleEvent(EVENT_ENERGY_SURGE, urand(1000, 4000));
            EventMap().ScheduleEvent(EVENT_BOON_OF_THE_SCORPID, 10000);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = EventMap().ExecuteEvent())
            {
                switch (eventId)
                {
                case EVENT_ENERGY_SURGE:
                    DoCastVictim(SPELL_ENERGY_SURGE);
                    EventMap().ScheduleEvent(EVENT_ENERGY_SURGE, urand(4000, 5000));
                    break;
                case EVENT_BOON_OF_THE_SCORPID:
                    DoCastSelf(SPELL_BOON_OF_THE_SCORPID, true);
                    EventMap().ScheduleEvent(EVENT_BOON_OF_THE_SCORPID, 8000);
                    break;
                default:
                    break;
                }
...
public:
    ...
    class spell_skorpyron_arcanoslash_damage_SpellScript : public SpellScript
    {
        ...
        void HandleHitTarget(SpellEffIndex /*effIndex*/)
        {
            ...
            if (target->ToPlayer() && target->ToPlayer()->GetRoleForGroup2() == ROLE_TANK)
            {
