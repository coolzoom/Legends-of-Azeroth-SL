...
public:
    ...
    {
        player->AddMovieDelayedAction2(MOVIE_LADY_KATHERINE, [player]()
        {
...
class aura_tol_dagor_intro_prisoner : public AuraScript
{
    ...
    {
        GetTarget()->RemoveGameObjectByEntry2(GOB_PRISON_BARS);
        GetTarget()->RemoveGameObjectByEntry2(GOB_PRISON_GATE);
...
struct quest_out_like_flynn : public QuestScript
{
    ...
    void OnQuestObjectiveChange(Player* player, Quest const* /*quest*/, QuestObjective const& objective, int32 /*oldAmount*/, int32 /*newAmount*/) override
    {
        ...
        {
           player->RemoveGameObjectByEntry2(GOB_PRISON_BARS);
           player->RemoveGameObjectByEntry2(GOB_PRISON_GATE);
        }
    }
};
...
struct npc_flynn_fairwind : public ScriptedAI
{
    ...
    void SpellHit(Unit* caster, SpellInfo const* spell) override
    {
        ...
        if (Creature* ashvaneJailer = GetAshvaneJailer())
        {
            ...
            {
                if (GameObject* gob = GetContextUnit()->GetGameObjectByEntry2(GOB_PRISON_BARS))
                    gob->UseDoorOrButton();
            })
                ...
                if (TempSummon* tempMe = me->ToTempSummon())
                    if (Unit* summoner = tempMe->GetSummoner())
                        if (Player* playerSummoner = summoner->ToPlayer())
                            playerSummoner->PlayConversation2(8334);
            }
            else
                context.Repeat();
        });
...
struct conversation_tol_dagor_inmate : public ConversationScript
{
    ...
    void OnConversationCreate(Conversation* conversation, Unit* creator) override
    {
        ...
        if (Unit* flynn = creator->GetSummonedCreatureByEntry2(NPC_FLYNN_ESCORT))
    ...
    void IsSummonedBy(WorldObject* unit) override
    {
        if (Player* player = unit->ToPlayer())
        {
                ...
                .Schedule(36s, [player](TaskContext /*context*/)
            {
                ...
                player->GetSceneMgr().PlayScene(1746);
            });
        }
    }
};
    ...
    void WaypointReached(uint32 waypointId, uint32 pointId) override
    {
        ...
        case 46:
          //  TalkToEscortPlayer(12);
            KillCreditMe2(GetPlayerForEscort());
            break;
        default:
            break;
        }
...
//135681
class npc_grand_admiral_jes_tereth_135681 : public PlayerScript
{
public:
    npc_grand_admiral_jes_tereth_135681() : PlayerScript("npc_grand_admiral_jes_tereth_135681") { }

    bool OnGossipHello(Player* player, Creature* creature)// override
    {
        AddGossipItemFor(player, GossipOptionIcon::None, "Set sail for Vol'dun.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 0);
        AddGossipItemFor(player, GossipOptionIcon::None, "Set sail for Nazmir.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        AddGossipItemFor(player, GossipOptionIcon::None, "Set sail for Zuldazar.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
