...
class npc_ji_firepaw_escort : public PlayerScript
{
public:
    npc_ji_firepaw_escort() : PlayerScript("npc_ji_firepaw_escort") { }
...
class npc_ji_firepaw : public PlayerScript
{
public:
    npc_ji_firepaw() : PlayerScript("npc_ji_firepaw")
    {
        isSummoned = false;
    }
    ...
    void SummonHiFirepawHelper(Player* summoner, uint32 entry)
    {
      ...
      //  if (!summon->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_UNIT), summoner->GetMap(), phase, entry, 0, team, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation()))
        ...
        summon->SetFollowAngle(summoner->GetAngle2(summon));
