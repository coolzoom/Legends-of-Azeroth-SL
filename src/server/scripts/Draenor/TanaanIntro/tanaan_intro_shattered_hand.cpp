...
public:
    ...
    bool OnQuestAccept(Player* player, Creature* creature, const Quest* quest)
    {
        switch (quest->GetQuestId())
        {
            ...
            {
                if (TempSummon* summon = player->SummonCreature(TanaanCreatures::NpcArchmageKhadgarSum, creature->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN, 0, 0))
                {
...
public:
    ...
    struct npc_shattered_hand_brawlerAI : public ScriptedAI
    {
        ...
        void IsSummonedBy(WorldObject* summoner) override
        {
            ...
            // Si il est déjr au sol, on ne le fait pas sauter, il passe directement r l'attaque sur un PNJ
        ...
        /*void MovementInform(uint32 type, uint32 id) override
        {
            ...
            if (id == 1)
            {
                ...
                {
                    // Pas de mob r attaquer, on le fait bouger un peu
