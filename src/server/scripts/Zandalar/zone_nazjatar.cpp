...
struct boss_wekemara : public BossAI
{
    ...
    {
        _DespawnAtEvade();
        me->DespawnCreaturesInArea2(NPC_SPAWN_OF_WEKEMARA, 125.0f);
        me->DespawnCreaturesInArea2(NPC_WEKEMARA_HANDLER, 125.0f);
...
struct npc_hidden_treasure : public ScriptedAI
{
    ...
    void Reset() override
    {
        me->SummonGameObject(GO_GLIMMERING_CHEST, me->GetPosition(), QuaternionData(), 60000);
    }
};

//322413
class go_glimmering_chest : public PlayerScript
{
public:
    go_glimmering_chest() : PlayerScript("go_glimmering_chest") { }
...
struct npc_unleashed_arcano_fiend : public ScriptedAI
{
    ...
    {
        ScriptedAI::Reset();
        me->DespawnCreaturesInArea2(153307);
    }
