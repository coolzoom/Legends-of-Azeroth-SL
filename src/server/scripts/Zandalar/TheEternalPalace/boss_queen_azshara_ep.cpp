...
struct noTanks : public std::unary_function<Unit*, bool>
{
    ...
    bool operator() (const Unit* pTarget)
    {
        ...
        uint32 specialization = player->GetSpecializationId2();
...
private:
    ...
    {
        AddGossipItemFor(player, GossipOptionIcon::None, "Attempt to activate the device.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 0);
...
struct bfa_npc_cyranus : public ScriptedAI
{
    ...
    void ExecuteEvent(uint32 eventId) //override
    {
        ...
        case EVENT_CHARGED_SPEAR:
        {
            ...
            Position pos;
            me->GetNearPoint(NULL, pos.m_positionX, pos.m_positionY, pos.m_positionZ, 1.0f, 180.0f);
...
struct bfa_at_piercing_gaze : AreaTriggerAI
{
    ...
    bool IsOnBeam(WorldObject* obj, Position source, Position destination, float heightDiff, float beamWidth, float beamLenght)
    {
        ...
        float angleSourceDest = source.GetAngle2(&destination);
        float angleSourcePoint = source.GetAngle2(obj);
