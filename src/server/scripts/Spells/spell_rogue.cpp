...
class spell_rog_slice_and_dice : public SpellScript
{
        ...
        if (Player* _player = GetCaster()->ToPlayer())
        {
            if (Aura* sliceAndDice = _player->GetAura(SPELL_ROGUE_SLICE_AND_DICE))
            {
                ...
                //if (SpellPowerCost const* powerCost = GetSpell()->GetPowerCost(POWER_COMBO_POINTS))
                    //five combo points : 36s
                    //if (powerCost->Amount == 5)
                        sliceAndDice->SetDuration(36 * IN_MILLISECONDS);
                //four combo points : 30s
                    //else if (powerCost->Amount == 4)
                        sliceAndDice->SetDuration(30 * IN_MILLISECONDS);
                //three combo points : 24s
                    //else if (powerCost->Amount == 3)
                        sliceAndDice->SetDuration(24 * IN_MILLISECONDS);
                //two combo points : 18s
                    //else if (powerCost->Amount == 2)
                        sliceAndDice->SetDuration(18 * IN_MILLISECONDS);
                //one combo point : 12s
                    //else if (powerCost->Amount >= 1)
    ...
    {
        OnHit += SpellHitFn(spell_rog_slice_and_dice::HandleOnHit);
    }
};
    ...
    class spell_rog_true_bearing_AuraScript : public AuraScript
    {
        ...
        void HandleProc(AuraEffect* /*aurEff*/, ProcEventInfo& /*eventInfo*/)
        {
            ...
            for (uint32 spell : spellIds)
            {
                ...
                // caster->GetSpellHistory()->ModifyCooldown(spell, -2000 * cp); not used anymore
